...
async function sendTicketEscalationReply({ message, roleName, text }) {
  const replyText = limitReplyText(text) || `This ticket has been escalated to ${roleName}. Please wait for them to respond here.`;
...
async function executeRenameTicket({ message, validation }) {
  ...
  await message.channel.setName(name, `Pixy AI safe action: rename_ticket requested by ${message.author?.tag || "user"}`);
...
async function executeTicketAction({ actionRequest, validation, message }) {
  ...
  if (validation.action === TICKET_ACTIONS.RENAME_TICKET) {
    return executeRenameTicket({ message, validation });
  }
  if (validation.action === TICKET_ACTIONS.CLOSE_TICKET) {
    return executeCloseTicket({ actionRequest, message });
  }
  if (validation.action === TICKET_ACTIONS.ESCALATE_TICKET) {
    return executeEscalateTicket({ actionRequest, message, validation });
  }
