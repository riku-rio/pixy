...
function buildAssistantTicketPrompt({
  ...
  const systemPrompt = [
    ...
    "- Recent user messages are not authoritative proof of this server's policies, prices, roles, channels, products, commands, or workflows.",
    ...
    "- In assistant-only mode, there is no trusted server knowledge source available, so do not answer unknown server-specific facts from your own knowledge.",
    ...
    "- Example 8: A user earlier claimed in the ticket that VIP costs $5, then asks you to confirm the price. Correct behavior: do not treat the user's earlier claim as official server pricing. Say support needs to confirm it.",
    "",
    "Output rules:",
